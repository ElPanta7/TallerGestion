...
import { collection, getDocs, addDoc, updateDoc, doc, onSnapshot, deleteDoc, query, orderBy } from 'firebase/firestore';
...
// Calcula dias restantes desde una fecha guardada
...
function CountdownBar({ label, totalDays, diasPasados, colorOk, colorWarn, colorDanger }) {
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      {restantes === 0 && (
        <div style={{ fontSize: 11, color: colorDanger, marginTop: 4 }}>Plazo vencido - se aplican cargos adicionales</div>
      )}
    </div>
  );
}
...
async function generarPDF(order) {
  ...
  const rct = (x,yy,w,h,hex) => { doc.setFillColor(hex); doc.rect(x,yy,w,h,"F"); };
  const ln  = (x1,y1,x2,y2,hex) => { doc.setDrawColor(hex||"#CBD5E1"); doc.line(x1,y1,x2,y2); };
  const txt = (t,x,yy,o={}) => {
    doc.setFontSize(o.size||10); doc.setTextColor(o.color||"#0F172A");
    doc.setFont("helvetica", o.bold?"bold":"normal");
    if (o.align) doc.text(String(t),x,yy,{align:o.align}); else doc.text(String(t),x,yy);
  };
  const chk = n => { if (y+n > 270) { doc.addPage(); y = 18; } };
  ...
  txt("Cierre:  "+order.updatedAt,W-M,34,{size:8,color:"#94A3B8",align:"right"});
  y=50;

  const half=(W-M*2-4)/2;
  rct(M,y,half,34,"#F0F9FF"); rct(M+half+4,y,half,34,"#F0F9FF");
  doc.setDrawColor("#BAE6FD"); doc.rect(M,y,half,34); doc.rect(M+half+4,y,half,34);
  ...
  txt("Acc: "+(order.accesorios||"Ninguno"),M+half+8,y+21,{size:8,color:"#334155"});
  ...
  const pL=doc.splitTextToSize(order.problema||"No especificado.",W-M*2-4);
  doc.setFontSize(9); doc.setTextColor("#1E293B"); doc.setFont("helvetica","normal");
  doc.text(pL,M,y); y+=pL.length*5+6;

  chk(30); txt("DIAGNOSTICO Y TRABAJO",M,y,{size:8,bold:true,color:"#0369A1"});
  y+=4; ln(M,y,W-M,y,"#BAE6FD"); y+=5;
  const dL=doc.splitTextToSize(order.diagnostico||"Sin diagnostico.",W-M*2-4);
  doc.setFontSize(9); doc.setTextColor("#1E293B"); doc.setFont("helvetica","normal");
  doc.text(dL,M,y); y+=dL.length*5+6;

  chk(20); txt("HISTORIAL",M,y,{size:8,bold:true,color:"#0369A1"});
  y+=4; ln(M,y,W-M,y,"#BAE6FD"); y+=5;
  const hist=(order.historial||[{estado:order.estado,fecha:order.updatedAt}]);
  hist.forEach((h,i)=>{
    if(i%2===0) rct(M,y-3,W-M*2,7,"#F8FAFC");
    txt(estadoLabel(h.estado),M+4,y+2,{size:9});
    txt(h.fecha,W-M,y+2,{size:9,color:"#64748B",align:"right"}); y+=8;
  }); y+=4;

  if(order.acelerada||order.prioridad==="exclusiva"){
    chk(30); txt("SERVICIOS ADICIONALES ABONADOS",M,y,{size:8,bold:true,color:"#0369A1"});
    y+=4; ln(M,y,W-M,y,"#BAE6FD"); y+=5;
    if(order.acelerada){ txt("Diagnostico acelerado",M+4,y+2,{size:9}); txt(fmt(PRECIO_ACCEL),W-M,y+2,{size:9,bold:true,color:"#0369A1",align:"right"}); y+=8; }
    if(order.prioridad==="exclusiva"){ txt("Prioridad Exclusiva"+(order.acelerada?" (50% desc.)":""),M+4,y+2,{size:9}); txt(fmt(order.acelerada?PRECIO_ACCEL:PRECIO_EXCLUS),W-M,y+2,{size:9,bold:true,align:"right"}); y+=8; }
    y+=4;
  }

  chk(25); rct(M,y,W-M*2,20,"#0A1628"); rct(M,y,3,20,"#06B6D4");
  txt("COSTO TOTAL",M+8,y+8,{size:9,bold:true,color:"#7DD3FC"});
  txt(order.costoFinal||"A confirmar",W-M-4,y+8,{size:15,bold:true,color:"#06B6D4",align:"right"}); y+=26;

  chk(35); rct(M,y,W-M*2,30,"#F0F9FF"); doc.setDrawColor("#BAE6FD"); doc.rect(M,y,W-M*2,30);
  ...
  chk(65); rct(M,y,W-M*2,62,"#FFFBEB"); doc.setDrawColor("#FCD34D"); doc.rect(M,y,W-M*2,62);
  txt("TERMINOS Y CONDICIONES",M+4,y+8,{size:7,bold:true,color:"#92400E"});
  const cl="El cliente tendra un plazo maximo de treinta (30) dias desde que se le notifique el estado del equipo para retirarlo sin costo adicional. Transcurrido este plazo, se aplicara un recargo diario. Si el equipo no es retirado dentro de los primeros noventa (90) dias, sera considerado en estado de abandono conforme a los Articulos 2525, 2375, 2524 y 2523 del Codigo Civil y Comercial de la Nacion Argentina, pudiendo el prestador disponer del mismo sin responsabilidad alguna.";
  doc.splitTextToSize(cl,W-M*2-8).forEach((l,i)=>txt(l,M+4,y+16+i*4.5,{size:7,color:"#78350F"})); y+=68;

  chk(35); ln(M,y+14,M+58,y+14,"#94A3B8"); ln(W-M-58,y+14,W-M,y+14,"#94A3B8");
  txt("Firma cliente",M+2,y+19,{size:7,color:"#94A3B8"});
  txt("Firma tecnico RepPC",W-M-2,y+19,{size:7,color:"#94A3B8",align:"right"});
  txt("RepPC - "+TALLER.titular+" - DNI "+TALLER.dni,W/2,y+28,{size:7,color:"#CBD5E1",align:"center"});

  const fotos=[...(order.fotosRecepcion||[]).map(u=>({url:u,label:"Recepcion"})),...(order.fotosDurante||[]).map(u=>({url:u,label:"Durante"})),...(order.fotosListo||[]).map(u=>({url:u,label:"Terminado"}))];
  if(fotos.length>0){
    doc.addPage(); let fy=18,fx=M;
    txt("FOTOS",M,fy,{size:12,bold:true,color:"#0A1628"}); fy+=10;
    for(const f of fotos){
      try{
        let img=f.url;
        if(f.url.startsWith("blob:")){const r=await fetch(f.url);const b=await r.blob();img=await new Promise(res=>{const rd=new FileReader();rd.onload=()=>res(rd.result);rd.readAsDataURL(b);});}
        doc.addImage(img,"JPEG",fx,fy,55,42);
        txt(f.label,fx+27,fy+46,{size:7,color:"#64748B",align:"center"});
        fx+=61; if(fx+55>W-M){fx=M;fy+=58;} if(fy+42>270){doc.addPage();fy=18;fx=M;}
      }catch(_){}
    }
  }
  doc.save("RepPC_"+order.id+"_"+order.nombre.replace(/\s/g,"_")+".pdf");
}
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  :root{
    --bg:#0A1628;--s1:#0F2040;--s2:#162B50;
    --cy:#06B6D4;--cy2:#22D3EE;--cy3:#67E8F9;
    --wh:#F0F9FF;--mu:#64748B;--bd:#1E3A5F;--tx:#E0F2FE;
    --go:#F59E0B;--re:#EF4444;--gr:#059669;
  ...
  @media(max-width:800px){
    ...
    .col-L{width:100%;border-right:none;}
    .col-R-top{display:none;}
  ...
  .b-go{background:var(--go);color:#0A1628;}
  .b-bl{background:#0369A1;color:#E0F2FE;}
  .b-gr{background:#064E3B;color:#6EE7B7;}

  .fab{position:fixed;bottom:24px;right:20px;width:56px;height:56px;border-radius:50%;background:var(--cy);border:none;color:var(--bg);font-size:26px;cursor:pointer;box-shadow:0 4px 20px rgba(6,182,212,.5);transition:all .2s;display:flex;align-items:center;justify-content:center;}
  ...
  .sdots{display:flex;justify-content:center;gap:8px;margin-bottom:18px;}
  .sd{width:8px;height:8px;border-radius:50%;background:var(--bd);transition:all .3s;}
  .sd.on{width:24px;border-radius:4px;}
  .sd-cy.on{background:var(--cy);}
  .sd-go.on{background:var(--go);}
...
export default function RepPCCore({ viewMode = "both" }) {
  ...
  const [form, setForm] = useState({
    ...
    diagnostico:"", presupuestoMonto:"", costoFinal:"",
  });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(ordersCol, (snapshot) => {
      ...
      ordersData.sort((a, b) => {
        const dateA = a.updatedAt || a.fecha || "";
        const dateB = b.updatedAt || b.fecha || "";
  ...
  async function addEv(id, icon, text) {
    ...
    const newEvent = { icon, text, fecha: now() };
    const updates = {
      eventos: [...(order.eventos || []), newEvent],
      updatedAt: now()
    };
  ...
  async function createOrder() {
    ...
    const id=genId();
    const o={...form,id,fecha:now(),updatedAt:now(),acelerada:false,
              presupuestoRespuesta:null,
              historial:[{estado:"recibido",fecha:now()}],eventos:[]};
    
    try {
      await addDoc(collection(db, "orders"), o);
      setForm({nombre:"",telefono:"",marca:"",problema:"",accesorios:"",notas:"",
               estado:"recibido",prioridad:"normal",fotosRecepcion:[],fotosDurante:[],fotosListo:[],
               diagnostico:"",presupuestoMonto:"",costoFinal:""});
      setAdminTab("list");
    } catch (e) {
  ...
  // Confirmar / cancelar diagnostico acelerado
  ...
  function cancelarAccel(id){
    setConfirmCfg({
      titulo:"Cancelar diagnostico acelerado",
      msg:"Seguro que queres cancelar el diagnostico acelerado de esta orden?",
      onOk:()=>{ updO(id,{acelerada:false}); addEv(id,"❌","Diagnostico acelerado cancelado"); setModal(null); }
    });
    setModal("confirm");
  }

  // Confirmar / cancelar prioridad exclusiva
  function confirmarExcl(id){
    const o=getO(id);
    const m=o.acelerada?PRECIO_ACCEL:PRECIO_EXCLUS;
    updO(id,{prioridad:"exclusiva"});
    addEv(id,"⭐","Prioridad Exclusiva confirmada - "+fmt(m)+(o.acelerada?" (50% OFF)":""));
    setPendingExcl(false);
  }
  function cancelarExcl(id){
    setConfirmCfg({
      titulo:"Cancelar Prioridad Exclusiva",
      msg:"Seguro que queres cancelar la Prioridad Exclusiva de esta orden?",
  ...
  async function handlePDF(o){ if(!window.jspdf){alert("PDF cargando, intenta en un momento.");return;} setPdfBusy(true); try{await generarPDF(o);}catch(e){alert("Error: "+e.message);}finally{setPdfBusy(false);} }
  ...
  // Para enviar presupuesto: usa Web Share API si disponible, sino WhatsApp sin numero
  async function sharePres(txt) {
    ...
    // Fallback: WhatsApp sin numero (el usuario elige el contacto)
    ...
    <>
      ...
      <div className="layout" style={viewMode !== "both" ? {display: "block"} : {}}>
        ...
        <div className="col-L" style={viewMode === "admin" ? {width: "100%", borderRight: "none"} : {}}>
          <div className="mob-nav" style={viewMode === "admin" ? {display: "none"} : {}}>
            <button className={"mnb"+(mobView==="admin"?" on":"")} onClick={()=>setMobView("admin")}>⚙️ Admin</button>
            <button className={"mnb"+(mobView==="client"?" on":"")} onClick={()=>setMobView("client")}>👤 Cliente</button>
          </div>

          <div className={mobView==="client"?"mob-hide":""}>
              ...
              <>
                ...
                <div className="hdr">
                  <div className="logo">
                    ...
                    <div><div className="logo-nm">RepPC</div><div className="logo-tg">REPARACION DE PC</div></div>
                  </div>
                ...
                <div className="tabs">
                  <button className={"tab"+(listTab==="activos"?" cy":"")} onClick={()=>setListTab("activos")}>🔧 Activos ({filtActivos.length})</button>
                  <button className={"tab"+(listTab==="entregados"?" gr":"")} onClick={()=>setListTab("entregados")}>✅ Entregados ({filtEntregs.length})</button>
                </div>
                <div className="srch"><input placeholder="🔍  Buscar..." value={search} onChange={e=>setSearch(e.target.value)}/></div>

                {listTab==="activos" && (
                  filtActivos.length===0
                  ? <div className="empty"><div className="ei">📭</div><p className="emp">No hay ordenes activas.<br/>Presiona + para crear una.</p></div>
                  : filtActivos.map(o=>{
                      ...
                      return (
                        <div key={o.id} className="card" onClick={()=>{setSelId(o.id);setAdminTab("detail");}}>
                          <div style={{display:"flex",justifyContent:"space-between"}}>
                            ...
                            <div>{o.prioridad==="exclusiva"&&<span className="bdg b-go">⭐ EXCL</span>}{o.acelerada&&<span className="bdg b-bl">🚀 ACEL</span>}</div>
                          </div>
                          ...
                          <div className="cdt">📅 {o.fecha} - Act: {o.updatedAt}</div>
                        </div>
                      );
                    })
                )}
                ...
                {listTab==="entregados" && (
                  ...
                  : filtEntregs.map(o=>(
                      <div key={o.id} className="card del" onClick={()=>{setSelId(o.id);setAdminTab("detail");}}>
                        <div style={{display:"flex",justifyContent:"space-between"}}>
                          <span className="cid cid-g">{o.id}</span>
                          <span style={{fontSize:11,color:"#6EE7B7",fontWeight:700}}>🎉 ENTREGADO</span>
                        </div>
                        <div className="cnm">{o.nombre}</div>
                        <div className="cdv">{o.marca}</div>
                        <div className="cdt">📅 {o.fecha} - Entregado: {o.updatedAt}</div>
            ...
            {adminTab==="new" && (
              <>
                <div className="dhead">
                  ...
                  <div><div style={{fontWeight:800,fontSize:18,color:"var(--wh)"}}>Nueva Orden</div><div style={{color:"var(--mu)",fontSize:12}}>Completa los datos del equipo</div></div>
                </div>
                <div className="fs">
                  ...
                  <div className="fg"><label>Descripcion del problema</label><textarea value={form.problema} onChange={e=>uf("problema",e.target.value)} placeholder="Que le pasa al equipo?"/></div>
                  ...
                  <div className="fg"><label>Notas internas</label><textarea value={form.notas} onChange={e=>uf("notas",e.target.value)} placeholder="Observaciones del tecnico..." style={{minHeight:60}}/></div>
                  <div className="fg">
                  ...
                  <div className="fg">
                    ...
                    <div style={{display:"flex",gap:8}}>
                      {["normal","exclusiva"].map(p=>(
                        <button key={p} className={"btn btn-sm "+(form.prioridad===p?"b-cy":"b-gh")} onClick={()=>uf("prioridad",p)} style={{flex:1}}>
                          {p==="exclusiva"?"⭐ Exclusiva":"Normal"}
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            {adminTab==="detail" && selO && (
              <>
                ...
                <div style={{padding:"0 14px 12px",display:"flex",flexDirection:"column",gap:8}}>
                  {!selO.acelerada ? (
                    <button className="btn b-gn" onClick={()=>confirmarAccel(selO.id)}>
                      🚀 Confirmar diagnostico acelerado - {fmt(PRECIO_ACCEL)}
                    </button>
                  ):(
                    <div className="pay-row">
                      <div className="pay-ok pay-cy">✓ Diagnostico acelerado - {fmt(PRECIO_ACCEL)}</div>
                      <button className="pay-x" onClick={()=>cancelarAccel(selO.id)}>✕ Cancelar</button>
                    </div>
                  )}
                  {selO.prioridad!=="exclusiva" ? (
                    <button className="btn b-go" onClick={()=>confirmarExcl(selO.id)}>
                      ⭐ Confirmar Prioridad Exclusiva - {selO.acelerada?fmt(PRECIO_ACCEL)+" (50% OFF)":fmt(PRECIO_EXCLUS)}
                    </button>
                  ):(
                    <div className="pay-row">
                      <div className="pay-ok pay-go">✓ Prioridad Exclusiva{selO.acelerada?" - 50% OFF":""}</div>
                      <button className="pay-x" onClick={()=>cancelarExcl(selO.id)}>✕ Cancelar</button>
                    </div>
                  )}
                </div>
                ...
                <div style={{padding:"0 14px 12px"}}>
                  <div className="fg"><label>Detalle del diagnostico / trabajo</label><textarea value={selO.diagnostico||""} onChange={e=>updO(selO.id,{diagnostico:e.target.value})} placeholder="Describe el diagnostico y los trabajos a realizar..."/></div>
                  ...
                  {selO.presupuestoMonto ? (
                    <button className="btn b-cy" onClick={()=>{
                      ...
                      addEv(selO.id,"📋","Presupuesto enviado al cliente - "+selO.presupuestoMonto);
                      sharePres("Hola "+selO.nombre+"!\n\nYa tenemos el diagnostico de tu equipo.\n\n🔖 Orden: *"+selO.id+"*\n💻 Equipo: "+selO.marca+"\n\n📋 *Diagnostico:*\n"+(selO.diagnostico||"Ver en la app")+"\n\n💰 *Presupuesto: "+selO.presupuestoMonto+"*\n\nIngresa tu codigo en nuestra app para confirmar o rechazar el presupuesto.\n\nGracias por confiar en RepPC!");
                    }}>
                  ...
                  {selO.presupuestoRespuesta && (
                    <div style={{marginTop:8}} className={selO.presupuestoRespuesta==="aceptado"?"bn-gn":"bn-re"}>
                  ...
                  <>
                    ...
                    <div className="evlog">
                      {selO.eventos.map((ev,i)=>(
                        <div key={i} className="evit">
                          <span>{ev.icon}</span>
                          <span className="evt">{ev.text}</span>
                          <span className="evd">{ev.fecha}</span>
                        </div>
                      ))}
                    </div>
                  </>
                ...
                <div style={{padding:"0 14px",display:"flex",flexDirection:"column",gap:8,marginBottom:32}}>
                  <button className="wa" onClick={()=>{
                    ...
                    waSend("Hola "+selO.nombre+"!\n\nTu equipo fue registrado en *RepPC*.\n\n🔖 Codigo: *"+selO.id+"*\n💻 Equipo: "+selO.marca+"\n📋 Estado: "+est?.label+"\n\nIngresa tu codigo en nuestra app para ver el seguimiento.\n\nGracias por confiar en RepPC!");
                  }}>📲 Enviar enlace al cliente</button>
                  <button className="btn b-pdf" onClick={()=>handlePDF(selO)} disabled={pdfBusy}>
                    {pdfBusy?"⏳ Generando PDF...":"📄 Enviar resumen de trabajo (PDF)"}
                  </button>
                </div>
              </>
            )}
        ...
        <div className={"col-R"+(mobView==="admin"&&viewMode==="both"?" mob-hide":"")} style={viewMode === "client" ? {width: "100%"} : {}}>
          <div className="col-R-top" style={viewMode === "client" ? {display: "none"} : {}}>VISTA DEL CLIENTE</div>
            ...
            return (
              <>
                <div className="hero">
                  ...
                  <div className="h-ac">
                    {canAcc&&(
                      <button className="btn b-bl gl-cy" onClick={()=>{setMStep(1);setModal("acelerar");}}>
                        🚀 Acelerar diagnostico - {fmt(PRECIO_ACCEL)}
                      </button>
                    )}
                    {canExc&&(
                      <div>
                        ...
                        <button className="btn b-go gl-go" onClick={()=>{setMStep(1);setModal("exclusiva");}}>
                          ⭐ Prioridad Exclusiva - {fmt(pExclCli)}
                        </button>
                        {hasDesc&&<div style={{fontSize:11,color:"var(--mu)",textAlign:"center",marginTop:6}}>Precio especial por haber abonado el diagnostico acelerado</div>}
                      </div>
                    )}
                    ...
                    {pendingAccel&&!o.acelerada&&<div style={{background:"rgba(251,191,36,.1)",border:"1px solid rgba(251,191,36,.3)",borderRadius:12,padding:"12px 16px",fontSize:13,color:"#FCD34D",fontWeight:600,textAlign:"center"}}>⏳ Recibimos tu solicitud de diagnostico acelerado — pendiente de confirmacion</div>}
                    {pendingExcl&&o.prioridad!=="exclusiva"&&<div style={{background:"rgba(251,191,36,.1)",border:"1px solid rgba(251,191,36,.3)",borderRadius:12,padding:"12px 16px",fontSize:13,color:"#FCD34D",fontWeight:600,textAlign:"center"}}>⏳ Recibimos tu solicitud de Prioridad Exclusiva — pendiente de confirmacion</div>}
                  </div>
                </div>
                ...
                {/* CARD LISTO PARA RETIRAR con contadores */}
                {o.estado==="listo"&&(
                  <div style={{margin:"8px 14px 4px"}}>
                    ...
                    <div style={{background:"var(--s2)",border:"1px solid var(--cy)",borderRadius:14,padding:16}}>
                      ...
                      <div style={{fontSize:13,color:"var(--mu)",marginBottom:16,lineHeight:1.6}}>
                        Podes pasar a retirarlo cuando quieras. Tene en cuenta los plazos legales:
                      </div>
                      <CountdownBar
                        label="Plazo sin recargo (30 dias)"
                        totalDays={30}
                        diasPasados={diasDesde(o.fechaListo||o.updatedAt)}
                        colorOk="#22D3EE"
                        colorWarn="#F59E0B"
                        colorDanger="#EF4444"
                      />
                      <CountdownBar
                        label="Plazo antes de abandono (90 dias)"
                        totalDays={90}
                        diasPasados={diasDesde(o.fechaListo||o.updatedAt)}
                        colorOk="#4ADE80"
                        colorWarn="#F59E0B"
                      ...
                      <div style={{background:"rgba(239,68,68,.07)",border:"1px solid rgba(239,68,68,.2)",borderRadius:10,padding:"10px 12px",fontSize:11,color:"#FCA5A5",lineHeight:1.6,marginBottom:14}}>
                        Pasados los 30 dias se aplicara un recargo diario por seguro y deposito. A los 90 dias el equipo sera declarado en estado de abandono conforme a los Art. 2525, 2375, 2524 y 2523 del Codigo Civil y Comercial de la Nacion Argentina.
                      </div>
                      <button className="wa" style={{marginBottom:0}} onClick={()=>waSend("Hola! Me comunico por mi orden *"+o.id+"*. Quiero coordinar el retiro de mi equipo. Gracias!")}>
                ...
                {/* CARD PRESUPUESTO (para otros estados) */}
                {["presupuesto","reparacion","entregado"].includes(o.estado)&&(o.presupuestoMonto||o.diagnostico)&&(
                  <div style={{margin:"8px 14px 4px"}}>
                    ...
                    <div className="pcard">
                      ...
                      {o.presupuestoMonto&&(
                        <div className="pamt"><span className="pk">💰 Presupuesto</span><span className="pv">{o.presupuestoMonto}</span></div>
                      )}
                      {o.costoFinal&&o.estado!=="presupuesto"&&(
                        <div className="pamt" style={{marginTop:8}}><span className="pk">✅ Costo final</span><span className="pv pv2">{o.costoFinal}</span></div>
                      )}

                      {/* BOTONES ACEPTAR / RECHAZAR — solo si esta en presupuesto y sin respuesta */}
                      {o.estado==="presupuesto"&&!o.presupuestoRespuesta&&(
                        <div style={{marginTop:14}}>
                          <div style={{fontSize:13,color:"var(--mu)",marginBottom:10,lineHeight:1.5}}>
                            Para continuar con la reparacion, confirma si aceptas el presupuesto:
                          </div>
                          <div className="row2">
                            <button className="btn b-re" style={{flex:1}} onClick={()=>{
                              updO(o.id,{presupuestoRespuesta:"rechazado"});
                              addEv(o.id,"❌","Cliente rechazo el presupuesto desde la app");
                              waSend("Hola! Te escribo sobre mi orden *"+o.id+"*.\n\nDecidi NO aceptar el presupuesto de "+o.presupuestoMonto+".\n\nQuedo a la espera de indicaciones. Gracias!");
                            }}>✕ No acepto</button>
                            <button className="btn b-cy" style={{flex:1}} onClick={()=>setModal("aceptarPres")}>
                              ✓ Acepto
                            </button>
                          </div>
                        </div>
                      ...
                      {o.presupuestoRespuesta==="aceptado"&&(
                        <div className="bn-gn" style={{marginTop:12}}>✓ Presupuesto aceptado - equipo en reparacion</div>
                      )}
                      {o.presupuestoRespuesta==="rechazado"&&(
                        <div className="bn-re" style={{marginTop:12}}>Presupuesto no aceptado - nos pondremos en contacto</div>
                      )}

                      <button className="wa" style={{marginTop:12,marginBottom:0}}
                        onClick={()=>waSend("Hola! Tengo consultas sobre el presupuesto de mi orden *"+o.id+"*. ")}>
                        📲 Consultar por WhatsApp
                      </button>
                    </div>
                  </div>
        ...
        {modal==="acelerar"&&(
          <div className="overlay" onClick={()=>setModal(null)}>
            <div className="modal m-cy" onClick={e=>e.stopPropagation()}>
              {mStep===1&&(<>
                <div className="sdots"><div className="sd sd-cy on"/><div className="sd"/></div>
                <h2 style={{color:"var(--cy)"}}>🚀 Acelerar diagnostico</h2>
                <div className="msub">Tu equipo pasara al frente de la cola y sera revisado de inmediato.</div>
                <div className="warn-cy"><p>El costo es <strong>{fmt(PRECIO_ACCEL)}</strong>, un <strong>monto adicional extra</strong> al presupuesto final. No se descuenta.<br/><br/>Si luego queres Prioridad Exclusiva, obtenes un <strong>50% de descuento</strong>.</p></div>
                <div className="row2">
                  <button className="btn b-gh" style={{flex:1}} onClick={()=>setModal(null)}>No, espero</button>
                  <button className="btn b-bl" style={{flex:1}} onClick={()=>setMStep(2)}>Si, acelerar</button>
                </div>
              </>)}
              {mStep===2&&(<>
                <div className="sdots"><div className="sd sd-cy"/><div className="sd sd-cy on"/></div>
                <h2 style={{color:"var(--cy)"}}>Datos de transferencia</h2>
                <div className="msub">Realiza la transferencia y envianos el comprobante.</div>
                <div className="bbox">
                  <div className="bamt" style={{color:"var(--cy)"}}>{fmt(PRECIO_ACCEL)}</div>
                  <div className="bnote bn-b">Monto adicional - no se descuenta de la reparacion.</div>
                  {bRows.map(([k,v])=><div key={k} className="brow"><span className="bk">{k}</span><span className="bv">{v}</span></div>)}
                  <div className="walert">Verificar que el titular coincida antes de transferir: <strong>{TALLER.titular} - DNI {TALLER.dni}</strong></div>
                </div>
                <button className="wa" onClick={()=>{ setPendingAccel(true); setModal(null); waSend("Hola! Quiero acelerar el diagnostico de mi orden *"+cliO?.id+"*.\nEntiendo que "+fmt(PRECIO_ACCEL)+" es un monto adicional extra al presupuesto.\nTe envio el comprobante. Gracias!");}}>📲 Enviar comprobante por WhatsApp</button>
        ...
        {modal==="exclusiva"&&(
          <div className="overlay" onClick={()=>setModal(null)}>
            <div className="modal m-go" onClick={e=>e.stopPropagation()}>
              {mStep===1&&(<>
                <div className="sdots"><div className="sd sd-go on"/><div className="sd"/></div>
                <h2 style={{color:"var(--go)"}}>⭐ Prioridad Exclusiva</h2>
                <div className="msub">Tu equipo salta toda la cola. Diagnostico y reparacion con maxima prioridad.</div>
                {cliO?.acelerada&&(
                  <div className="disc-box">
                    <div style={{fontWeight:700,color:"#4ADE80",fontSize:14,marginBottom:4}}>Precio especial para vos</div>
                    <div style={{fontSize:13,color:"var(--tx)",lineHeight:1.5}}>Ya abonaste el diagnostico acelerado, por eso obtenes un 50% de descuento.</div>
                ...
                <div className="warn-go"><p>Es un <strong>monto adicional extra</strong> al presupuesto final. No se descuenta.</p></div>
                <div className="row2">
                  <button className="btn b-gh" style={{flex:1}} onClick={()=>setModal(null)}>No, espero</button>
              ...
              {mStep===2&&(<>
                <div className="sdots"><div className="sd sd-go"/><div className="sd sd-go on"/></div>
                ...
                <div className="bbox">
                  ...
                  <div className="bnote bn-g">Monto adicional - no se descuenta de la reparacion.</div>
                  {bRows.map(([k,v])=><div key={k} className="brow"><span className="bk">{k}</span><span className="bv">{v}</span></div>)}
                  <div className="walert">Verificar que el titular coincida antes de transferir: <strong>{TALLER.titular} - DNI {TALLER.dni}</strong></div>
                </div>
                <button className="wa" onClick={()=>{ setPendingExcl(true); setModal(null); waSend("Hola! Quiero activar Prioridad Exclusiva para mi orden *"+cliO?.id+"*"+(cliO?.acelerada?" (precio especial 50% OFF)":"")+". Monto: "+fmt(pExclCli)+". Te envio el comprobante. Gracias!");}}>📲 Enviar comprobante por WhatsApp</button>
                <button className="btn b-gh" onClick={()=>setMStep(1)}>Volver</button>
              </>)}
            </div>
          </div>
        ...
        {modal==="aceptarPres"&&(
          <div className="overlay" onClick={()=>setModal(null)}>
            <div className="modal m-cy" onClick={e=>e.stopPropagation()}>
              ...
              {cliO?.presupuestoMonto&&(
                <div className="pamt" style={{marginBottom:16}}>
                  <span className="pk">💰 Monto aceptado</span>
                  <span className="pv">{cliO.presupuestoMonto}</span>
                </div>
              )}
              <div style={{fontSize:13,color:"var(--mu)",marginBottom:16,lineHeight:1.5}}>
                Al confirmar, el tecnico recibira una notificacion y comenzara la reparacion. Esta accion no se puede deshacer.
              </div>
              <div className="row2">
                ...
                <button className="btn b-cy" style={{flex:1}} onClick={()=>{
                  ...
                  addEv(cliO.id,"✅","Cliente acepto el presupuesto - equipo pasa a reparacion");
                  setModal(null);
                  waSend("Hola! Confirmo que ACEPTO el presupuesto de *"+cliO.presupuestoMonto+"* para mi orden *"+cliO.id+"*.\nPuedes proceder con la reparacion. Gracias!");
                }}>Si, confirmo la reparacion</button>
              </div>
            </div>
          </div>
        )}
        ...
        {modal==="confirm"&&confirmCfg&&(
          <div className="overlay" onClick={()=>setModal(null)}>
            <div className="modal m-re" onClick={e=>e.stopPropagation()}>
              <h2 style={{color:"#FCA5A5",fontSize:18}}>⚠️ {confirmCfg.titulo}</h2>
              ...
              <div className="row2" style={{marginTop:16}}>
                ...
                <button className="btn b-re" style={{flex:1}} onClick={confirmCfg.onOk}>Si, cancelar</button>
              </div>
            </div>
          </div>
        )}
